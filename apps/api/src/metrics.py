"""Prometheus metrics registry for bsopt."""
...
PRICE_MAPE_GAUGE = Gauge(
    "bsopt_price_mape_percent", "Mean Absolute Percentage Error", ["method_type"]
)

# ── NeonDB ────────────────────────────────────────────────────────
NEON_QUERY_DURATION = Histogram(
    "bsopt_neondb_query_duration_seconds",
    "Duration of NeonDB queries",
    ["operation"],
...
SCRAPE_ROWS_INSERTED = Gauge(
    "bsopt_scrape_rows_inserted", "Total rows inserted by scrapers", ["market"]
)
...
WS_CONNECTIONS_ACTIVE = Gauge(
    "bsopt_ws_connections_active", "Total active WebSocket connections", ["channel"]
)
