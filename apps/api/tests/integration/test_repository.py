...
@pytest.mark.asyncio
async def test_user_lifecycle(db_cleanup: Any) -> None:
    ...
    async with acquire() as conn:
        await conn.execute(
            "INSERT INTO users (id, email, role) VALUES ($1, $2, $3)", str(user_id), email, "admin"
        )
...
@pytest.mark.asyncio
async def test_option_parameters_upsert(db_cleanup: Any) -> None:
...
@pytest.mark.asyncio
async def test_audit_log(db_cleanup: Any) -> None:
