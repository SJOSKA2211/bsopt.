...
def test_analytical_call(base_params: OptionParams) -> None:
...
def test_analytical_put(base_params: OptionParams) -> None:
...
def test_explicit_fdm_cfl_violation(base_params: OptionParams) -> None:
    pricer = ExplicitFDM()
    with pytest.raises(CFLViolationError):
        pricer.price(base_params, m_steps=200, n_steps=100)


def test_explicit_fdm_put(base_params: OptionParams) -> None:
    params = OptionParams(100, 100, 1, 0.2, 0.05, "put")
    pricer = ExplicitFDM()
...
def test_implicit_fdm_put(base_params: OptionParams) -> None:
    params = OptionParams(100, 100, 1, 0.2, 0.05, "put")
    pricer = ImplicitFDM()
...
def test_crank_nicolson_put(base_params: OptionParams) -> None:
    params = OptionParams(100, 100, 1, 0.2, 0.05, "put")
...
def test_standard_mc_confidence(base_params: OptionParams) -> None:
...
def test_antithetic_mc_odd_paths(base_params: OptionParams) -> None:
...
def test_antithetic_mc_put(base_params: OptionParams) -> None:
    params = OptionParams(100, 100, 1, 0.2, 0.05, "put")
    pricer = AntitheticMonteCarlo()
...
def test_binomial_crr_american(base_params: OptionParams) -> None:
    pricer = BinomialCRR()
    # European call
...
def test_implicit_fdm_american_put(base_params: OptionParams) -> None:
...
def test_crank_nicolson_american_put(base_params: OptionParams) -> None:
    params_amer = OptionParams(100, 100, 1, 0.2, 0.05, "put", exercise_type="american")
    pricer = CrankNicolsonFDM()
    res = pricer.price(params_amer, m_steps=100, n_steps=1000)
    assert pytest.approx(res.computed_price, 0.1) == 6.09
...
def test_trinomial_american_put(base_params: OptionParams) -> None:
    params_euro = OptionParams(100, 100, 1, 0.2, 0.05, "put", exercise_type="european")
    params_amer = OptionParams(100, 100, 1, 0.2, 0.05, "put", exercise_type="american")
    pricer = TrinomialTree()
    res_euro = pricer.price(params_euro, num_steps=200)
    res_amer = pricer.price(params_amer, num_steps=200)
    assert res_amer.computed_price > res_euro.computed_price
    assert pytest.approx(res_amer.computed_price, 0.05) == 6.09
...
@pytest.mark.parametrize(
    ...
    [
        ...
        CrankNicolsonFDM,
        StandardMonteCarlo,
        AntitheticMonteCarlo,
        ControlVariateMonteCarlo,
        QuasiMonteCarlo,
        BinomialCRR,
        TrinomialTree,
        RichardsonExtrapolation,
...
def test_all_methods_agreement(pricer_class: type, base_params: OptionParams) -> None:
    pricer = pricer_class()
    ...
    elif isinstance(pricer, ControlVariateMonteCarlo):
        res = pricer.price(base_params, num_paths=20000, num_steps=100)
    else:
        res = pricer.price(base_params)

    bs_price = 10.4505
    tol = 0.2 if "MonteCarlo" in pricer.__class__.__name__ else 0.1
    # Control Variate with 100 steps still has a small bias vs continuous BS Asian,
    # so we allow a bit more room or we could use more steps.
    if isinstance(pricer, ControlVariateMonteCarlo):
        tol = 0.5

    assert pytest.approx(res.computed_price, abs=tol) == bs_price


def test_analytical_geometric_asian(base_params: OptionParams) -> None:
    pricer = BlackScholesAnalytical()
...
def test_control_variate_mc_convergence(base_params: OptionParams) -> None:
...
def test_explicit_fdm_american_put(base_params: OptionParams) -> None:
    params_amer = OptionParams(100, 100, 1, 0.2, 0.05, "put", exercise_type="american")
    pricer = ExplicitFDM()
    # Need enough steps for stability
    res = pricer.price(params_amer, m_steps=50, n_steps=2000)
    # American put should be approx 6.09
    assert pytest.approx(res.computed_price, 0.1) == 6.09


def test_analytical_geometric_asian_put(base_params: OptionParams) -> None:
    params = OptionParams(100, 100, 1, 0.2, 0.05, "put")
    pricer = BlackScholesAnalytical()
...
def test_option_params_invalid_exercise() -> None:
    with pytest.raises(ValueError, match="Exercise type"):
        OptionParams(100, 100, 1, 0.2, 0.05, "call", exercise_type="invalid")


def test_quasi_mc_sobol_paths(base_params: OptionParams) -> None:
