...
async def test_push_notifications_paths(db_cleanup) -> None:
    ...
    async with run_fake_server(port=8085):
        ...
        # Trigger broad Exception branch (Line 52-53)
        sub_invalid_key = {
            "endpoint": "http://127.0.0.1:8085/push",
            "keys": {
                "auth": "invalid", 
                "p256dh": "invalid_p256dh"  # This should cause cryptography to raise ValueError/Exception
            }
        }
        await save_user_push_subscription(user_id, json.dumps(sub_invalid_key))
        res = await send_push_notification(n)
